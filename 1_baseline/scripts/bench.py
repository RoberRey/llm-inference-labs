...
class RequestResult:
    ...
    Attributes:
        prompt_len (int): Number of tokens in the input prompt (approx).
        out_len (int): Number of tokens requested in the output.
...
async def run_one_request(
    ...
    Args:
        ...
        prompt_len (int): Prompt length in tokens (approximate).
    ...
    stream = await client.chat.completions.create(
        ...
        messages=[
            ...
            {
                'role': 'user',
                'content': f"Repeat the word 'hello' {out_len} times.",
            },
        ],
...
async def run_config(
    ...
    """
    print(
        f'[bench] prompt_len={prompt_len}, out_len={out_len}, '
    ...
    while remaining > 0:
        ...
        tasks = [
            asyncio.create_task(
                run_one_request(client, model, prompt_len, out_len, concurrency)
            )
...
def aggregate(results: List[RequestResult]) -> List[Dict[str, Any]]:
    ...
    Groups results by (prompt_len, concurrency).

    Args:
        results (List[RequestResult]): Recorded per-request metrics.

    Returns:
        List[Dict[str, Any]]: Summary metrics with percentiles and throughput.
    """
    rows: List[Dict[str, Any]] = []

    key_to_results: Dict[tuple, List[RequestResult]] = {}
    for r in results:
        key = (r.prompt_len, r.concurrency)
        key_to_results.setdefault(key, []).append(r)

    for (prompt_len, concurrency), group in sorted(key_to_results.items()):
        ttfts = np.array([g.ttft for g in group], dtype=float)
        lats = np.array([g.latency for g in group], dtype=float)

        total_tokens_per_req = prompt_len + group[0].out_len
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    Loads the client, runs multiple configurations, aggregates results,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Prompt lengths to test (in 'tokens', approximate)",
    )
    parser.add_argument(
        '--out-len',
        type=int,
